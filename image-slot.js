...
(() => {
  ...
  const stylesheet =
    ...
    // compact: dense contexts (table rows, list items) where the full
    // icon + "or browse files" stack cannot fit. Keeps the caption only.
    ':host([compact]) .empty{padding:2px;gap:0}' +
    ':host([compact]) .empty svg,:host([compact]) .empty .sub{display:none}' +
    ':host([compact]) .empty .cap{font:600 10px/1 ui-monospace,SFMono-Regular,monospace;' +
    '  letter-spacing:.06em;max-width:100%}' +
